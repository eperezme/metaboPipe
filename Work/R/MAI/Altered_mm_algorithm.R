# Altered NSKNN mixed missingness function
removeDataMM_altered <- function(data,
                                 percentMiss,
  ...
  # # of samples for each metabolite that belong to the low abundance group of
  # metabolites that will be missing.
  numSampBelowAbundThreshLowAbund <- round(
    percentMisslowAbund_III / 100 * ncol(lowAbundMet)
  )
  if (numSampBelowAbundThreshLowAbund == 0) {
    dw <- 0
  } else {
    dw <- 1
  }
  ...
  if (alwaysMV < ncol(LowLowAbund)) {
    ...
    temp[removeIdx] <- NA
    LowLowAbundRandomMV <- matrix(temp,
      nrow = nrow(LowLowAbundRandomMV),
      ncol = ncol(LowLowAbundRandomMV), byrow = TRUE
    )
    LowLowAbund <- cbind(LowLowAbund, LowLowAbundRandomMV)
  }
  lowMNARidx <- which(is.na(LowLowAbund), arr.ind = TRUE)
  ...
  if (numSampBelowAbundThreshMedAbund == 0) {
    dz <- 0
  } else {
    dz <- 1
  }
  ...
  LMedMNARidx <- which(is.na(LowMedAbund), arr.ind = TRUE)
  ...
  remainingValuesVector <- c(
    as.vector(t(aboveThreshWithMV)),
