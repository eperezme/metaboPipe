MAI <- function(data_miss, MCAR_algorithm = c("BPCA", "Multi_nsKNN", "random_forest", "mean", "median", "QRILC", "SVD", "PPCA", "nsKNN", "Single"),
                ...
                n_cores = 1, assay_ix = 1, forest_list_args = list(
                  ntree = 300,
                  proximity = FALSE
                ), verbose = TRUE) # nolint
{
  ...
  tryCatch(
    {
      smallestDistance <- threshs[[which.min(lapply(
        threshs,
        function(x) {
          unlist(x[["distance"]])
        }
  ...
  rfFit <- suppressMessages(do.call(train, c(list(
    form = target ~
  ...
  if (exists("fldata")) {
    ...
    return(list(Imputed_data = Imputed_data[["MAI"]], Estimated_Params = list(
      Alpha = alpha,
