...
check_distance <- function(data_miss, data_sub, PercentMiss) {
  ...
  return(list(
    thresh = threshs[which.min(distances)],
...
generate_predictors <- function(missingData, labels = TRUE) {
  ...
  num_missing <- apply(
    apply(missingData, 2, as.numeric),
    1, function(x) {
      sum(is.na(x))
    }
  )
  ...
  frame.as.vector[, seq_len(6)] <- apply(
    frame.as.vector[, seq_len(6)],
    2, as.numeric
  )
...
imputation_algorithms <- function(missingData,
  ...
  if (MCAR_algorithm == "Single") {
    MCAR_imputation <- kapImpute2(missingData)
  }
  if (MCAR_algorithm == "nsKNN") {
    k <- floor(sqrt(nrow(missingData)))
    if (k > numcols) {
      k <- numcols - 1
    }
    MCAR_imputation <- nsKNN(missingData, k, iters = 1)
  }
  ...
  if (MCAR_algorithm == "PPCA") {
    result <- pca(missingData, method = "ppca")
    MCAR_imputation <- completeObs(result)
  }
  ...
  if (MNAR_algorithm == "BPCA") {
    MNAR_imputation <- pca(missingData,
      method = "bpca",
      verbose = FALSE
    )@completeObs
  }
  if (MNAR_algorithm == "random_forest") {
    MNAR_imputation <- missForest(missingData)[["ximp"]]
  }
  if (MNAR_algorithm == "Multi_nsKNN") {
    k <- floor(sqrt(nrow(missingData)))
    if (k > numcols) {
      k <- numcols - 1
    }
    MNAR_imputation <- nsKNN(missingData, k,
      iters = 4, weighted = TRUE, scale = TRUE,
      shuffle = TRUE
    )
  }
  if (MNAR_algorithm == "mean") {
    MNAR_imputation <- colMeans(missingData, na.rm = TRUE)
  }
  if (MNAR_algorithm == "median") {
    MNAR_imputation <- apply(missingData, 2, median, na.rm = TRUE)
  }
  if (MNAR_algorithm == "QRILC") {
    MNAR_imputation <- apply(missingData, 2, function(x) quantile(x, .25, na.rm = TRUE))
  }
  if (MNAR_algorithm == "SVD") {
    MNAR_imputation <- svdImpute(missingData, ...)$imputed
  }
  if (MNAR_algorithm == "BPCA") {
    MNAR_imputation <- pca(missingData, method = "bpca", verbose = FALSE)@completeObs
  }
  if (MNAR_algorithm == "PPCA") {
    result <- pca(missingData, method = "ppca")
    MNAR_imputation <- completeObs(result)
  }
