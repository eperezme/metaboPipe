...
Per a carregar les dades s'han d'importar dues taules en format \texttt{.csv} anomenades \texttt{dataMatrix} i \texttt{sampleMetadata} que contenen les dades d'abundancia relativa o concentracio  (\Cref{tab:dataMatrix-example}) i les dades referent a les mostres respectivament (\Cref{tab:sampleMetadata-example}). 
...
Per filtrar els valors faltants, s'ha decidit utilitzar per defecte la regla del 80\% modificada, és a dir, si una columna o filera té més del 20\% de valors faltants, aquesta columna o filera serà eliminada. L'usuari pot decidir manualment el \% de valors faltants que vol permetre.


Per a filtrar els outliers s'ha decidit utilitzar una Hotelling's T2 distribution ellipse amb un nivell de significacio per defecte del 95\%, ajustable manualment per l'usuari al 99\% si aixi desitja. S'ha decidit utilitzar aquest mètode donat que és robust i flexible per a la detecció d'outliers en dades multivariades, pren en compte la covariància entre les variables \cite{wuUsingNontargetedLCMS2021,viallonNewPipelineNormalization2021}.
